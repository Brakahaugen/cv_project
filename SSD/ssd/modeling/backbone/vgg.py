...
class VGG(nn.Module):
    def __init__(self, cfg):
        super().__init__()
#         size = cfg.INPUT.IMAGE_SIZE[0]
        vgg_config = vgg_base[str(cfg.INPUT.IMAGE_SIZE[0])]
        extras_config = extras_base[str(cfg.INPUT.IMAGE_SIZE[1])]

        self.vgg = nn.ModuleList(add_vgg(vgg_config))
        self.extras = nn.ModuleList(add_extras(extras_config, i=1024, size=(cfg.INPUT.IMAGE_SIZE[1])))
